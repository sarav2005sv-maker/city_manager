...
void add_command(int argc, char** argv, int role){
  ...
  int monitor_pid = -1;
    ...
    if (pid_fd != -1) {
        char pid_buf[SIZE];
        ssize_t bytes_read = read(pid_fd, pid_buf, SIZE - 1);
        close(pid_fd);
    ...
    if (signal_sent) {
        const char *success_msg = "Monitor notified successfully via SIGUSR1.\n";
        write(fd3, success_msg, strlen(success_msg));
    } else {
        const char *fail_msg = "Monitor could not be informed of the event (Monitor not running or error).\n";
        write(fd3, fail_msg, strlen(fail_msg));
    }
