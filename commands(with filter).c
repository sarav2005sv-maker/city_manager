...
void write_to_log(int permissions, int role, char** argv, int fd3){
  if (role == MANAGER && (permissions & S_IWUSR) != 0){
    ...
    fprintf(stderr, "Permission denied - Trying to write to logged_district as INSPECTOR\n");
  }
}
...
/**
 * Splits a condition string "field:operator:value" into three parts.
 * [cite: 65, 66]
 */
